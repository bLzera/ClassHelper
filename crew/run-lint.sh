...
# Arquivos .rb alterados pelo épico, relativos a backend/ (rubocop roda de dentro de backend/).
mapfile -t CHANGED < <(git -C "$ROOT" diff --name-only main...HEAD -- 'backend/**/*.rb' \
  | sed 's#^backend/##')
